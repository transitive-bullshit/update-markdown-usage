...
const main = (module.exports = async (argv) => {
  program
    ...
    .option(
      '-f, --file <filename>',
      'markdown file to update (defaults to local readme)'
    )
    ...
    .option(
      '-s, --section <string>',
      'name of markdown section to update',
      (s) => s,
      'usage'
    )
    .option(
      '-W, --no-write',
      'write result to stdout (defaults to updating markdown file)'
    )
    .parse(argv)
  ...
  if (program.stdin) {
    ...
    const binary = Object.values(pkg.packageJson.bin || {})[0]
  ...
  const output = updateMarkdownSection(
    markdownContent,
    sectionContent,
    program.section
  )
